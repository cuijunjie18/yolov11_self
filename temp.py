...
def letterbox(img, new_shape=(640, 640), color=(114, 114, 114), auto=False, scaleFill=False, scaleup=True):
    ...
    # 缩放后的未填充尺寸
    new_unpad = (int(round(shape[1] * r)), int(round(shape[0] * r)))  # (new_h,new_w)
...
def visualize(image_path, boxes, scores, class_ids, class_names):
    """
    输入
    image_path : 图片路径
    boxes : 预测框(在原始图像尺度上的)
...
if __name__ == '__main__':
    ...
    # 示例：可视化结果
    class_names = [str(i) for i in range(100)]  # 替换为你的类别名称
    visualize(img_path, boxes, scores, class_ids, class_names)